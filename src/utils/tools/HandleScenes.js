...
export default class HandleScenes {
  ...
  onAttach(){
    ...
    if(this.webgl.scene.anim) this.nbStepsCam = this.webgl.scene.anim.keyframes.length
  ...
  onChangeSceneStep(){
    this.currentStepCam = (this.webgl.scene.anim.currentKeyfame + 1 ) % this.nbStepsCam;
  ...
  getCurrentStepTexts(){
    ...
    let result = currentSceneTexts.filter(el => el.camera == this.currentStepCam)
